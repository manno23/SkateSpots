...
public class ActionBarHelperBase extends ActionBarHelper {
    ...
    private void setupActionBar() {
        ...
        homeItem.setIcon(R.drawable.skate_spots_logo_altered);



        // Add title Image
        ImageButton titleText = new ImageButton(mActivity, null, R.attr.actionbarCompatItemStyle);
        titleText.setLayoutParams(springLayoutParams);
        titleText.setScaleType(ImageView.ScaleType.FIT_START);
        //Adjust Padding to pixel density
        if(mActivity.getWindowManager().getDefaultDisplay().getHeight() <= 320)
            titleText.setPadding(9, 9, 3, 7);
        else
            titleText.setPadding(16, 14, 10, 11);
        titleText.setImageResource(R.drawable.skate_spots_logo_altered);
    ...
    public void setAddSpotItemState(boolean ready) {
        View addSpotReadyButton = mActivity.findViewById(R.id.actionbar_compat_add_spot_ready);
        View addSpotNotReadyButton = mActivity.findViewById(R.id.actionbar_compat_add_spot_not_ready);

        if (addSpotNotReadyButton != null) {
            Log.d("Application", "not ready, not null");
            addSpotNotReadyButton.setVisibility(ready ? View.GONE : View.VISIBLE);
        }
        if (addSpotReadyButton != null) {
            Log.d("Application", "ready, not null");
            addSpotReadyButton.setVisibility(ready ? View.VISIBLE : View.GONE);
        }
    }
    ...
    private View addActionItemCompatFromMenuItem(final MenuItem item) {
        ...
        // These are added to create a dual/state for the add spot
        // action bar button
        if (itemId == R.id.menu_add_spot_ready) {
            actionButton.setId(R.id.actionbar_compat_add_spot_ready);
            actionButton.setVisibility(View.GONE);
        }
        if (itemId == R.id.menu_add_spot_not_ready) {
            actionButton.setId(R.id.actionbar_compat_add_spot_not_ready);
